...
(push :emma *features*)  ;;mjs indicate emma module
...
(defmethod move-attention ((eye-mod emma-vis-mod) &key location scale)
  ...
  (if (eq (exec-s eye-mod) 'BUSY)
    ...
    (progn
      ...
      (setf (moving-attention eye-mod) (gentemp "TRGT")) 
...
;;;   emma code
      (let ((return-obj (get-obj-at-location eye-mod location scale)))
        (if return-obj
          ...
          ;; If there's nothing there now, then either assume the last
          ;; target if it was set or assume the default shift time
          (if (shift-target eye-mod)
            ...
            (let* ((start-loc (next-destination eye-mod))
                   ...
                   (r-theta (compute-saccade-r-theta start-loc end-loc)))
              (change-state eye-mod :proc 'BUSY)
              (setf (shift-duration eye-mod) (recog-time eye-mod nil r-theta))
              (initiate-eye-move eye-mod (shift-duration eye-mod) r-theta)
...
(defmethod encoding-complete ((eye-mod emma-vis-mod) (loc-dmo dmo) scale)
  (declare (symbol scale))
  (setf (moving-attention eye-mod) NIL)
  (change-state eye-mod :exec 'free :proc 'FREE)
  (let ((return-obj
         (if (and (shift-target eye-mod)
                  (object-present-p eye-mod (id (shift-target eye-mod))))
           (shift-target eye-mod)
           (get-obj-at-location eye-mod loc-dmo scale))))
    (unless return-obj
      (clear-attended eye-mod)
      (setf (attend-failure eye-mod) t)
      (return-from encoding-complete nil))
    (set-attended eye-mod return-obj)
    (when (print-view-p eye-mod)
...
(defmethod update-attended-loc ((eye-mod emma-vis-mod))
  ...
  (when (or (and (currently-attended eye-mod)
                 (not (object-present-p eye-mod 
                                        (id (currently-attended eye-mod)))))
            (and (current-marker eye-mod)
                 (null (currently-attended eye-mod))
                 (within-move eye-mod (current-lof eye-mod))))
        (schedule-event-relative 0 'move-attention  ;;;mjs   
                                 :params (list 
                                            eye-mod
                                            :location (dmo-to-psdme (current-marker eye-mod))
  ...
  (let ((sacc-mvmt (build-sacc-mvmt eye-mod r-theta)))
    ...
    (when (<= recog-time (total-time sacc-mvmt))
     (schedule-event-relative (max 0 recog-time)  'ENCODING-COMPLETE   ;;mjs
                             ...
                             :output 'medium
                             :details (concatenate 'string "Encoding-Complete " (symbol-name (id (current-marker eye-mod)))))
)))
...
(defmethod complete-eye-move ((eye-mod emma-vis-mod) (trgt symbol) 
  ...
  (when (and (moving-attention eye-mod)
    ...
    (let* ((start-loc (eye-loc eye-mod))
           (end-loc (dmo-to-xy (current-marker eye-mod)))
           (r-theta (compute-saccade-r-theta start-loc end-loc))
           (new-duration
...
(defmethod recog-time ((eye-mod emma-vis-mod) (obj dmo) (r-theta vector))
  (let ((freq (object-frequency eye-mod obj))
        (eccentricity (pm-pixels-to-angle (vr r-theta))))
    (rand-time (* (enc-factor eye-mod)
                  (- (log freq))
                  (exp (* eccentricity (enc-exponent eye-mod)))))))

(defmethod recog-time ((eye-mod emma-vis-mod) (obj list) (r-theta vector))
  (let ((freq (object-frequency eye-mod obj))
        (eccentricity (pm-pixels-to-angle (vr r-theta))))
    (rand-time (* (enc-factor eye-mod)
                  (- (log freq))
...
(defmethod recog-time ((eye-mod emma-vis-mod) (obj dmo) (r-theta list))
...
(defmethod object-frequency ((eye-mod emma-vis-mod) (obj dmo)
                                &optional (default 0.01))
  (let ((value (get-attribute obj 'value)))
    (if (stringp value)
        (aif (gethash value (freq-ht eye-mod))
             it
             default)
      (aif (feat-named eye-mod (id obj))
           (let ((val (obj-freq it)))
             (setf (obj-freq it) (+ (obj-freq it) 0.01))
             val)
           default))))


(defmethod object-frequency ((eye-mod emma-vis-mod) (obj list)
                                &optional (default 0.01))
           default)
...
#+(and :lispworks :macosx)  ;;mjs
(progn
  
  (defclass eye-spot ()
    ((color :accessor color :initarg :color :initform (color:make-rgb 0.8 0.2 0.2 1.0))
     (offset :accessor offset :initarg :offset :initform nil)
     (view-size :accessor view-size :initarg :view-size :initform nil))
    (:default-initargs 
      :view-size (vector 9 9) 
      :offset (vector -5 -5)))
...
(define-module-fct :vision 
  ...
  (list 
   (define-parameter :conservative-update-visual
     ...
     :default-value nil
     :warning "T or NIL"
     :documentation "Re-use old visual objects?")
...
;;;emma stuff
  ...
  :version "2.3a2-emma"
  :documentation "Vision-module with EMMA"
  :creation #'create-emma-module ;;;#'create-vision-module
  :reset #'reset-emma-module ;;;#'reset-vision-module
  ...
  :params #'params-emma-module ;;;#'params-vision-module
