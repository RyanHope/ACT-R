...
(defmethod device-handle-click ((device #-(version>= 7) cg:window #+(version>= 7) cg:basic-pane))
  ...
  (let (x y)
    ...
    (when (and (show-focus-p (current-device-interface))
               (equal (type-of *attn-tracker*) 'focal-view))
      (setf *attn-tracker* (cg:make-window :focus-ring :device 'focal-view :parent device
                                      :left x 
                                      :top y )))
      )
...
;;;; ---------------------------------------------------------------------- ;;;;;;;
;;;; ACL screen-to-icon interface
;;;; ---------------------------------------------------------------------- ;;;;;;;


;;; BUILD-FEATURES-FOR      [Method]
;;; Description : For an ACL window, just walk the sub-objects (which by
;;;             : default are subviews) and build features.

(defmethod build-features-for ((self #-(version>= 7) cg:window #+(version>= 7) cg:basic-pane) (vis-mod vision-module))
  (let ((base-ls (flatten
...
(defmethod build-features-for ((self cg:dialog-item) 
                                  (vis-mgr vision-module))
...
(defmethod build-features-for ((self cg:editable-text)
...
(defmethod build-features-for ((self cg:button)
                                  (vis-mod vision-module))
  "Builds an icon feature for a BUTTON DIALOG-ITEM"
...
(defmethod build-features-for ((self cg:static-text)
                                  (vis-mod vision-module))
  (let* ((font-spec (cg:nfontmetrics (cg:window self) (cg:make-fontmetrics)))
         (ascent (cg:font-ascent font-spec))
         (descent (cg:font-descent font-spec))
...
(defmethod build-features-for :around ((self cg:static-text)
                                          (vis-mod vision-module))
...
(defmethod approach-width ((feat text-feature) (theta number))
  (let ((screen-obj (screen-obj feat)))
    (if (and screen-obj
             (eq 'button (class-name (class-of screen-obj))))
      (let ((new-feat (first (member screen-obj (visicon (get-module :vision))
                                       :key #'screen-obj))))
        (if new-feat
...
(defmethod cursor-to-feature ((the-window #-(version>= 7) cg:window #+(version>= 7) cg:basic-pane))
  ...
  (let ((pos (cg:cursor-position the-window))
    ...
    (when (cursor-in-window-p the-window)
      (make-instance 'cursor-feature
        :x (cg:position-x pos) :y (cg:position-y pos)
...
#| Moved to vision for general usage
...
(defmethod fill-default-dimensions ((feat icon-feature))
  "Fill in the width, height, and size of an icon feature."
...
(defmethod device-update-attended-loc ((wind #-(version>= 7) cg:window #+(version>= 7) cg:basic-pane) xyloc)
  (unless (or (equal t *attn-tracker*)
              #+:allegro-v5.0.1 (equal (type-of *attn-tracker*) 'closed-stream)
              #+(version>= 6) (null (cg:handle wind))
              (equal wind (cg:parent *attn-tracker*)))
    
    (close *attn-tracker*))
  (when (or (eql t *attn-tracker*)
            #+:allegro-v5.0.1 (equal (type-of *attn-tracker*) 'closed-stream)
            #+(version>= 6) (null (cg:handle *attn-tracker*))
            )
    
    (setf *attn-tracker* (cg:make-window :focus-ring :device 'focal-view :parent wind
                                      :left (- (px xyloc) 11) 
                                      :top (- (py xyloc) 11))))
