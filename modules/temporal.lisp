...
;; dummy function for event trace
...
(defun temporal-request (instance buffer-name chunk-spec)
  ...
  (let ((ticks (chunk-spec-slot-spec chunk-spec 'ticks))
    ...
    (if (or ticks clear)
        (if clear
            (schedule-event-now 'temporal-clear :module 'temporal :details "Clear")
          (if (not (zerop (verify-single-explicit-value chunk-spec 'ticks 'temporal 'time)))
