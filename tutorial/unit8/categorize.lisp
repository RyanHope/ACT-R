...
(defun present-one-attribute-fct (name value)
  (goal-focus-fct (car (define-chunks-fct `((state add-attribute name ,name value ,(+ *attribute-offset* value))))))
  (run 20))
...
(defun run-trial (stim)
  ...
  (let ((goal (car (define-chunks (state categorize)))))
    (goal-focus-fct goal)
    (run 20)
...
(define-model categorize
  ...
  (set-buffer-chunk 'imaginal (car (define-chunks (category unknown))))
